...
public class RowMapper {
    ...
    public List<Row> convert(JenkinsBuildDistribution input, Filter filter) {
    ...
    List<Row> rows = new ArrayList<>();
        ...
        for (JenkinsJob job : jobs) {
            ...
            for (JenkinsBuild build : builds) {
                ...
                Row row = new Row();
                row.setHost(getBuildOn(build));
                row.setJobName("<h6>" + job.getName() + "</h6><br>Result:" + build.getResult());

                row.setFrom(build.getTimestamp());
                row.setTo(build.getTimestamp() + build.getDuration());
